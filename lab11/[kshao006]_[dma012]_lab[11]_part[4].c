...
const unsigned long periodKey = 50;
const unsigned long periodShow = 500;
...
int TickFct_show(int state)
{
	...
	switch (state) {
		...
		case show_s2:
		if((currKey >= 48 && currKey <= 57) || currKey == 35 || currKey == 42 ||(currKey >= 65 && currKey <= 68)){
			LCD_Cursor(show_counter);
